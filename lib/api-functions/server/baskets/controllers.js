...
const updateUserBasket = async (req, res) => {
  const owner = req?.user?.sub;
  const basketItems = req.body;
  console.log(basketItems);
  //create basketdata from basketData
  ...
  try {
    const basket = await getUserBasketFromDB(owner, true);
    // eslint-disable-next-line no-underscore-dangle
    console.log('basket from user already', basket);
    ...
    console.error('error updating users basket', err);
